...
namespace Beetle
{
	...
	void EditorLayer::OnImGuiRender()
	{
		...
		ImGui::Text("Hovered : %s", name.c_str());
	...
	void EditorLayer::OnEvent(Event &e)
	{
		...
		dispatcher.Dispatch<MouseButtonPressedEvent>(BEETLE_BIND_EVENT_FN(EditorLayer::OnMouseButtonPressed));
	}
	...
	void EditorLayer::OnOverlayRender()
	{
		...
		if (Entity selectedEntity = m_SceneHierarchyPanel.GetSelectedEntity())
		{
			...
			Renderer2D::DrawRect(transform.GetTransform(), glm::vec4(1.0f, 0.2f, 0.8f, 1.0f));
		}
	...
	void EditorLayer::OpenScene(const std::filesystem::path& path)
	{
		...
		{
			BT_WARN("Could not load {0} - not a scene file", path.filename().string());
			return;
		}
